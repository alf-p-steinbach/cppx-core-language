...
|------|---------|---------------|
| Declaration statements| “declarations.hpp” | `using` names from a given namespace unqualified, e.g. via `$use_std` and `$use_cppx`; declaring aliases of nested namespaces without repeating the names; declaring tag types; and `$with` for executing a block with a variable scoped to that block, like the C# and Java auto cleanup constructs. This is all necessarily macro-based. One wishes for ch-ch-ch-changes. |
