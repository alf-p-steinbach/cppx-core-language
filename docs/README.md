...
The syntax support is mainly about reducing excessive verbosity and “repeat yourself!”-requirements in the core language and its direct library support. This is organized in 5 areas, each in a sub-folder with a small set of headers. Each sub-folder *X* has a corresponding header “*X*.hpp” that includes everything from that sub-folder, and an exporting namespace `cppx:`*X*.

| Area | *X*.hpp | Functionality |
|------|---------|---------------|
| Declaration statements| “declarations.hpp” | Declaring namespace aliases without repeating the names; declaring tag types; `using` names from a given namespace unqualified; and `$with` for executing a block with a variable scoped to that block, like the C# and Java auto cleanup constructs. This is all necessarily macro-based. One wishes for ch-ch-ch-changes. |
...
| Creating or assembling `std::string`s | “string-expressions.hpp” | A `<<` notation in order to assemble text pieces and io-streamable values, which can be very convenient for function arguments, e.g. for `fail`; operator `*` to specify *n* repeats of a string; and pseudo-operator `^sz` to call `.c_str()`. |
